...
// Validate checks the field values on TopicWithFetchedTasks with the rules
...
// ValidateAll checks the field values on TopicWithFetchedTasks with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// TopicWithFetchedTasksMultiError, or nil if none found.
func (m *TopicWithFetchedTasks) ValidateAll() error {
	return m.validate(true)
}

func (m *TopicWithFetchedTasks) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	for idx, item := range m.GetTasks() {
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, TopicWithFetchedTasksValidationError{
			...
			if err := v.Validate(); err != nil {
				return TopicWithFetchedTasksValidationError{
					field:  fmt.Sprintf("Tasks[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if len(errors) > 0 {
		return TopicWithFetchedTasksMultiError(errors)
	}

	return nil
}

// TopicWithFetchedTasksMultiError is an error wrapping multiple validation
// errors returned by TopicWithFetchedTasks.ValidateAll() if the designated
// constraints aren't met.
type TopicWithFetchedTasksMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m TopicWithFetchedTasksMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m TopicWithFetchedTasksMultiError) AllErrors() []error { return m }

// TopicWithFetchedTasksValidationError is the validation error returned by
// TopicWithFetchedTasks.Validate if the designated constraints aren't met.
...
// Field function returns field value.
func (e TopicWithFetchedTasksValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e TopicWithFetchedTasksValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e TopicWithFetchedTasksValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e TopicWithFetchedTasksValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e TopicWithFetchedTasksValidationError) ErrorName() string {
	return "TopicWithFetchedTasksValidationError"
}

// Error satisfies the builtin error interface
func (e TopicWithFetchedTasksValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sTopicWithFetchedTasks.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = TopicWithFetchedTasksValidationError{}
...
var _ interface {
	...
	Cause() error
	ErrorName() string
} = TopicWithFetchedTasksValidationError{}
